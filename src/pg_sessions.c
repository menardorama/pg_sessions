 ...
 *
 * IDENTIFICATION
 *	  contrib/pg_sessions/pg_sessions.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
#define PGSS_DUMP_FILE	PGSTAT_STAT_PERMANENT_DIRECTORY "/pg_sessions.stat"
 ...
 * as part of changing the GUC, but pg_sessions has no way of avoiding
...
PG_FUNCTION_INFO_V1(pg_sessions_reset);
PG_FUNCTION_INFO_V1(pg_sessions_1_2);
PG_FUNCTION_INFO_V1(pg_sessions_1_3);
PG_FUNCTION_INFO_V1(pg_sessions);
...
static void pg_sessions_internal(FunctionCallInfo fcinfo,
...
_PG_init(void)
{
	 ...
	 * allow the pg_sessions functions to be created even when the
	 ...
	 */
	DefineCustomIntVariable("pg_sessions.max",
	  "Sets the maximum number of statements tracked by pg_sessions.",
							NULL,
							&pgss_max,
							5000,
							100,
	...
	DefineCustomEnumVariable("pg_sessions.track",
	...
	DefineCustomBoolVariable("pg_sessions.track_utility",
	   "Selects whether utility commands are tracked by pg_sessions.",
							 NULL,
							 &pgss_track_utility,
							 true,
							 PGC_SUSET,
	...
	DefineCustomBoolVariable("pg_sessions.save",
			   "Save pg_sessions statistics across server shutdowns.",
							 NULL,
							 &pgss_save,
							 true,
							 PGC_SIGHUP,
	...
	EmitWarningsOnPlaceholders("pg_sessions");
...
pgss_shmem_startup(void)
{
	...
	pgss = ShmemInitStruct("pg_sessions",
	...
	pgss_hash = ShmemInitHash("pg_sessions hash",
							  pgss_max, pgss_max,
							  &info,
...
fail:
	 ...
	 * server is running with pg_sessions enabled
	 */
}
...
pgss_ExecutorStart(QueryDesc *queryDesc, int eflags)
{
	...
	if (pgss_enabled() && queryDesc->plannedstmt->queryId != 0)
	{
		...
		if (queryDesc->totaltime == NULL)
		{
	        ...
	        if (queryDesc->plannedstmt->queryId != 0 && queryDesc->totaltime && pgss_enabled())
	        {
	                /*
 ...
 */
Datum
pg_sessions_reset(PG_FUNCTION_ARGS)
{
	if (!pgss || !pgss_hash)
		ereport(ERROR,
				(errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
				 errmsg("pg_sessions must be loaded via shared_preload_libraries")));
	entry_reset();
	PG_RETURN_VOID();
}
 ...
 */
Datum
pg_sessions_1_3(PG_FUNCTION_ARGS)
{
	bool		showtext = PG_GETARG_BOOL(0);

	pg_sessions_internal(fcinfo, PGSS_V1_3, showtext);

	return (Datum) 0;
}

Datum
pg_sessions_1_2(PG_FUNCTION_ARGS)
{
	bool		showtext = PG_GETARG_BOOL(0);

	pg_sessions_internal(fcinfo, PGSS_V1_2, showtext);

	return (Datum) 0;
}

/*
 * Legacy entry point for pg_sessions() API versions 1.0 and 1.1.
 * This can be removed someday, perhaps.
 */
Datum
pg_sessions(PG_FUNCTION_ARGS)
{
	/* If it's really API 1.1, we'll figure that out below */
	pg_sessions_internal(fcinfo, PGSS_V1_0, true);

	return (Datum) 0;
}

/* Common code for all versions of pg_sessions() */
static void
pg_sessions_internal(FunctionCallInfo fcinfo,
							pgssVersion api_version,
							bool showtext)
{
